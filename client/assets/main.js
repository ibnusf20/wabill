...
let formatData = () => {
	...
	if (dataIsValid) {
		...
		toast("masukan data terlebih dahulu", "danger")
		editData()
	}
}
...
let previewMessage = () => {
	if (!dataIsFormatted) {
		toast("anda belum memasukan format pesan", "danger")
	} else {
		formatData()
...
// var array = ['some', 'array', 'containing', 'words'];
	var interval = 4000 // how much time should the delay between two iterations be (in milliseconds)?
		  ...
		  promise = promise.then(function () {
		    ...
		    fetch(document.getElementById("server").value + "send-message", {
				...
				.then(res => {
				  ...
				  if (res.status) {
				  	updateStatus(i - 1, "Terkirim")
				  } else {
				  	updateStatus(i - 1, "Nomor tidak terdaftar")
...
let clearConsole = () => {
	console.clear()
	// made with <3 by ibnusf20 | visit me on https://ibnusf20.github.io/
	console.log('%cmade with <3 by ibnusf20', 'font-weight: bold; font-size: 30px;color: #ff2600; text-shadow: 1px 1px 0px black , -1px -1px 0px white')
	console.log('%cvisit me on https://ibnusf20.github.io/', 'font-weight: bold; font-size: 20px;color: black; text-shadow: 1px 1px 0px #fc0 , -1px -1px 0px white')
}